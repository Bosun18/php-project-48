    ...
    $result = array_map(
        function ($key) use ($data1, $data2) {
            return match (true) {
                ...
                ['key' => $key, 'type' => 'nested', 'value1' => getTree($data1[$key], $data2[$key]),
                    'value2' => getTree($data1[$key], $data2[$key]),],
                !array_key_exists($key, $data2) =>
                ['key' => $key, 'type' => 'deleted', 'value1' => $data1[$key], 'value2' => null,],
                !array_key_exists($key, $data1) =>
                ['key' => $key, 'type' => 'added', 'value1' => null, 'value2' => $data2[$key],],
                $data1[$key] !== $data2[$key] =>
                ['key' => $key, 'type' => 'updated', 'value1' => $data1[$key], 'value2' => $data2[$key],],
                default =>
                ['key' => $key, 'type' => 'immutable', 'value1' => $data1[$key], 'value2' => $data2[$key],],
            };
        },
        $keys
    );
